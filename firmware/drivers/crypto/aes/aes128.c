// FIPS-197 AES-128 single-block implementation. Compact and dependency-free so
// it builds identically on the host test suite and on device.
#include "aes128.h"

#include <string.h>

static const uint8_t kSBox[256] = {
    };

static uint8_t kInvSBox[256];
static int kInvReady = 0;

static void ensureInvSBox(void) {
  if (kInvReady) return;
  for (int i = 0; i < 256; i++) kInvSBox[kSBox[i]] = (uint8_t)i;
  kInvReady = 1;
}

static const uint8_t kRcon[11] = {0x00,0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1b,0x36};

// GF(2^8) multiply by 2 (xtime).
static uint8_t xtime(uint8_t x) { return (uint8_t)((x << 1) ^ ((x >> 7) * 0x1b)); }

static uint8_t gmul(uint8_t a, uint8_t b) {
  uint8_t p = 0;
  for (int i = 0; i < 8; i++) {
    if (b & 1) p ^= a;
    uint8_t hi = a & 0x80;
    a <<= 1;
    if (hi) a ^= 0x1b;
    b >>= 1;
  }
  return p;
}

// Expand a 16-byte key into 11 round keys (176 bytes).
static void expandKey(const uint8_t key[16], uint8_t rk[176]) {
  memcpy(rk, key, 16);
  int bytes = 16;
  int rconIdx = 1;
  uint8_t tmp[4];
  while (bytes < 176) {
    memcpy(tmp, &rk[bytes - 4], 4);
    if (bytes % 16 == 0) {
      uint8_t t = tmp[0];
      tmp[0] = (uint8_t)(kSBox[tmp[1]] ^ kRcon[rconIdx++]);
      tmp[1] = kSBox[tmp[2]];
      tmp[2] = kSBox[tmp[3]];
      tmp[3] = kSBox[t];
    }
    for (int i = 0; i < 4; i++) {
      rk[bytes] = (uint8_t)(rk[bytes - 16] ^ tmp[i]);
      bytes++;
    }
  }
}

static void addRoundKey(uint8_t s[16], const uint8_t* rk) {
  for (int i = 0; i < 16; i++) s[i] ^= rk[i];
}

void corefw_aes128_encrypt_block(const uint8_t key[16], const uint8_t in[16], uint8_t out[16]) {
  uint8_t rk[176];
  expandKey(key, rk);
  uint8_t s[16];
  memcpy(s, in, 16);

  addRoundKey(s, rk);
  for (int round = 1; round <= 10; round++) {
    // SubBytes
    for (int i = 0; i < 16; i++) s[i] = kSBox[s[i]];
    // ShiftRows (state is column-major: s[r + 4c])
    uint8_t t;
    t = s[1]; s[1] = s[5]; s[5] = s[9]; s[9] = s[13]; s[13] = t;
    t = s[2]; s[2] = s[10]; s[10] = t; t = s[6]; s[6] = s[14]; s[14] = t;
    t = s[15]; s[15] = s[11]; s[11] = s[7]; s[7] = s[3]; s[3] = t;
    // MixColumns (skip on final round)
    if (round != 10) {
      for (int c = 0; c < 4; c++) {
        uint8_t* col = &s[c * 4];
        uint8_t a0 = col[0], a1 = col[1], a2 = col[2], a3 = col[3];
        col[0] = (uint8_t)(xtime(a0) ^ (xtime(a1) ^ a1) ^ a2 ^ a3);
        col[1] = (uint8_t)(a0 ^ xtime(a1) ^ (xtime(a2) ^ a2) ^ a3);
        col[2] = (uint8_t)(a0 ^ a1 ^ xtime(a2) ^ (xtime(a3) ^ a3));
        col[3] = (uint8_t)((xtime(a0) ^ a0) ^ a1 ^ a2 ^ xtime(a3));
      }
    }
    addRoundKey(s, &rk[round * 16]);
  }
  memcpy(out, s, 16);
}

void corefw_aes128_decrypt_block(const uint8_t key[16], const uint8_t in[16], uint8_t out[16]) {
  ensureInvSBox();
  uint8_t rk[176];
  expandKey(key, rk);
  uint8_t s[16];
  memcpy(s, in, 16);

  addRoundKey(s, &rk[10 * 16]);
  for (int round = 9; round >= 0; round--) {
    // InvShiftRows
    uint8_t t;
    t = s[13]; s[13] = s[9]; s[9] = s[5]; s[5] = s[1]; s[1] = t;
    t = s[2]; s[2] = s[10]; s[10] = t; t = s[6]; s[6] = s[14]; s[14] = t;
    t = s[3]; s[3] = s[7]; s[7] = s[11]; s[11] = s[15]; s[15] = t;
    // InvSubBytes
    for (int i = 0; i < 16; i++) s[i] = kInvSBox[s[i]];
    addRoundKey(s, &rk[round * 16]);
    // InvMixColumns (skip on the round that mirrors the final encrypt round)
    if (round != 0) {
      for (int c = 0; c < 4; c++) {
        uint8_t* col = &s[c * 4];
        uint8_t a0 = col[0], a1 = col[1], a2 = col[2], a3 = col[3];
        col[0] = (uint8_t)(gmul(a0, 14) ^ gmul(a1, 11) ^ gmul(a2, 13) ^ gmul(a3, 9));
        col[1] = (uint8_t)(gmul(a0, 9) ^ gmul(a1, 14) ^ gmul(a2, 11) ^ gmul(a3, 13));
        col[2] = (uint8_t)(gmul(a0, 13) ^ gmul(a1, 9) ^ gmul(a2, 14) ^ gmul(a3, 11));
        col[3] = (uint8_t)(gmul(a0, 11) ^ gmul(a1, 13) ^ gmul(a2, 9) ^ gmul(a3, 14));
      }
    }
  }
  memcpy(out, s, 16);
}
